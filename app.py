...
COLUMN_RULES = {
    ...
    # 流量/销售核心字段
    '7天销售额': {'exact': ['7天销售额', '销售额(7天)'], 'fuzzy': ['7 days sales']},
    '14天销售额': {'exact': ['14天销售额', '销售额(14天)'], 'fuzzy': ['14 days sales']},
    '7天订单商品总数': {'exact': ['7天订单商品总数', '7天订单', '订单(7天)'], 'fuzzy': ['7天销量']},
    '14天订单商品总数': {'exact': ['14天订单商品总数', '14天订单', '订单(14天)'], 'fuzzy': ['14天销量']},
    '广告花费': {'exact': ['广告花费'], 'fuzzy': ['ad spend', 'spend (ad)']},
    '广告销售额': {'exact': ['广告销售额'], 'fuzzy': ['ad sales']},
    '广告订单': {'exact': ['广告订单量', '广告订单'], 'fuzzy': ['ad orders']},
    '广告点击数': {'exact': ['广告点击数', '广告点击'], 'fuzzy': ['ad clicks']},
    '广告曝光量': {'exact': ['广告曝光量', '广告展示量', '广告展示', '广告曝光'], 'fuzzy': ['ad impressions']},
    '订单毛利润': {'exact': ['订单毛利润', '毛利润', '毛利额'], 'fuzzy': ['profit']},
}

def smart_find_column(df, key):
    """根据规则智能定位列名，返回第一个匹配的列名或None"""
        ...
        with st.spinner("🧠 正在执行引擎重构，挂载多维数据穿透与排雷网..."):
            ...
            if df_wl is not None:
                if '店铺' in df_wl.columns and '店铺' in df_master.columns:
                    ...
                    mask = merged['_shop_norm_x'].str.contains(merged['_shop_norm_y'], regex=False) | \
                           merged['_shop_norm_y'].str.contains(merged['_shop_norm_x'], regex=False)
            ...
            sku_agg = merged_agg.groupby('MSKU', dropna=False).agg({'预测日销量': 'sum', '总供给': 'first'}).rename(columns={'预测日销量': 'SKU_总日均', '总供给': 'SKU_总供给'})
            merged_agg = pd.merge(merged_agg, sku_agg, on='MSKU', how='left')
            ...
            # 展示层去重：只保留每个MSKU的第一行显示库存，其他行置零
            merged_agg = merged_agg.sort_values(by=['MSKU', '7天销售额'], ascending=[True, False]).reset_index(drop=True)
            is_dup = merged_agg.duplicated(subset=['MSKU'], keep='first')
            ...
            for c in [c for c in clr_cols if c in merged_agg.columns]:
                merged_agg.loc[is_dup, c] = 0
            merged_agg.loc[is_dup, '预计可售天数'] = None
            ...
            merged_agg.drop(columns=['SKU_总日均', 'SKU_总供给', 'SKU_KEY'], errors='ignore', inplace=True)

            # 列排序
            mv_front = [c for c in ['预测日销量', '建议补货量', '预计可售天数', '理论需求量', '总供给'] if c in merged_agg.columns]
            others = [c for c in merged_agg.columns if c not in mv_front]
...
if "df_vis" in st.session_state:
    ...
    g_p, g_m, g_ac, g_ap, g_s, g_sd = calc_kpi(df_vis, '7天')  # 大盘用7天基准
    ...
    st.markdown("---")
    st.download_button(label="📥 下载完整【V42·彻底优化版.xlsx】", data=st.session_state.processed_excel,
