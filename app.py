...
def to_numeric_fast(series):
    ...
    s = np.where(has_pct, s / 100.0, s)
    s = np.where(is_neg, -np.abs(s), s)
...
if run_btn:
    if not all_files: st.error("❌ 请上传数据表格！")
        ...
        with st.spinner("🧠 正在执行防御性引擎重构，挂载多维数据穿透与动态排雷网..."):
            ...
            def merge_traffic(m_df, t_df):
                ...
                m_df, t_df = m_df.loc[:, ~m_df.columns.duplicated()].copy(), t_df.loc[:, ~t_df.columns.duplicated()].copy()
                t_cols = [c for c in t_df.columns if c not in ['join_key', 'traffic_shop']]
                
                # 防御场景1：真实数据不被0占位符清除
                overlap = [c for c in t_cols if c in m_df.columns]
                for col in overlap:
                    if pd.to_numeric(m_df[col], errors='coerce').fillna(0).abs().sum() == 0:
                        m_df.drop(columns=[col], inplace=True)
                    else:
                        t_df.drop(columns=[col], inplace=True)
                ...
                if '店铺' in temp.columns and 'traffic_shop' in temp.columns:
                    sc = temp.groupby('MSKU')['店铺'].transform('count')
                    for idx, row in temp.iterrows():
                        m_s, t_s = str(row['店铺']).upper().replace(" ", ""), str(row['traffic_shop']).upper().replace(" ", "")
                        if t_s not in ['UNKNOWN', 'NAN']:
                            if t_s not in m_s and m_s not in t_s:
                                for col in t_cols:
                            ...
                            if sc.iloc[idx] > 1:
                                for col in t_cols:
                                    if col in temp.columns and pd.notna(temp.at[idx, col]):
                                        temp.at[idx, col] = float(temp.at[idx, col]) / sc.iloc[idx]
                                        
                grp_cols = [c for c in m_df.columns if c not in t_cols]
                return temp.drop(columns=['join_key', 'traffic_shop'], errors='ignore').groupby(grp_cols, dropna=False)[t_cols].sum().reset_index()
...
if "df_vis" in st.session_state:
    ...
    st.markdown("---")
    st.download_button(label="📥 下载完整【V42·动态防御最终版.xlsx】", data=st.session_state.processed_excel, file_name=f"V42_安全护盾大盘_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary")
