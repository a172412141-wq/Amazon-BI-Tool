...
# ⚡️ 核心提速与防错：强制正则提取纯数字，彻底根除 $ 和 % 导致的字符串相加 Bug
    ...
    if not all_files: st.error("❌ 请上传数据表格！")
        ...
        with st.spinner("🧠 正在执行全息穿透洗数引擎，请稍候..."):
            ...
            def merge_traffic(m_df, t_df):
                ...
                # 防重血统拦截：主表已有核心财务数据，踢除流量表同名列，防止污染
            ...
            # 重算比例
            def _s_div(n, d): return np.where(merged[d] > 0, merged[n] / merged[d], 0)
            if '广告花费' in merged.columns:
                if '广告销售额' in merged.columns: merged['ACOS'] = _s_div('广告花费', '广告销售额')
                if '广告点击数' in merged.columns: merged['CPC'] = _s_div('广告花费', '广告点击数')
                if '7天销售额' in merged.columns: merged['ACoAS'] = _s_div('广告花费', '7天销售额')
            if '广告订单' in merged.columns and '广告点击数' in merged.columns: merged['广告CVR'] = _s_div('广告订单', '广告点击数')
            if '广告点击数' in merged.columns and '广告曝光量' in merged.columns: merged['CTR'] = _s_div('广告点击数', '广告曝光量')
            if '订单毛利润' in merged.columns and '7天销售额' in merged.columns: merged['订单毛利率'] = _s_div('订单毛利润', '7天销售额')
            ...
            merged['7天销售转化率'] = _s_div('7天订单商品总数', '7天会话数') if '7天会话数' in merged.columns else 0
            merged['14天销售转化率'] = _s_div('14天订单商品总数', '14天会话数') if '14天会话数' in merged.columns else 0
...
if "df_vis" in st.session_state:
    ...
    opt_p = sorted(df_f[df_f[spu_c].astype(str).str.strip() != ''][spu_c].dropna().unique().tolist()) if spu_c else []
    opt_a = sorted(df_f[df_f['ASIN'].astype(str).str.strip() != '']['ASIN'].dropna().unique().tolist()) if 'ASIN' in df_f.columns else []
    opt_m = sorted(df_f[df_f['MSKU'].astype(str).str.strip() != '']['MSKU'].dropna().unique().tolist()) if 'MSKU' in df_f.columns else []
    ...
    def s_sum(df, col): return float(pd.to_numeric(df[col], errors='coerce').sum()) if col in df.columns else 0.0
    def calc_kpi(df):
        ...
        if not spu_c: st.warning("⚠️ 未找到 SPU 列")
            ...
            if is_filtered:
                ...
                for c in [f'{prefix}销售额', '订单毛利润', '广告花费', '广告销售额', '可用量', '待到合计', '建议补货量', f'{prefix}日均订单']:
                    if c in s_cols: t_row[c] = pd.to_numeric(target_df[c], errors='coerce').sum()
                ...
                sp_df['订单毛利率'] = np.where(sp_df['7天销售额']>0, sp_df['订单毛利润']/sp_df['7天销售额'], 0)
                sp_df['ACOS'] = np.where(sp_df['广告销售额']>0, sp_df['广告花费']/sp_df['广告销售额'], 0)
                sp_df['ACoAS'] = np.where(sp_df[f'{prefix}销售额']>0, sp_df['广告花费']/sp_df[f'{prefix}销售额'], 0)
                sp_df['CPC'] = np.where(sp_df['广告点击数']>0, sp_df['广告花费']/sp_df['广告点击数'], 0)
                sp_df['广告CVR'] = np.where(sp_df['广告点击数']>0, sp_df['广告订单']/sp_df['广告点击数'], 0)
                sp_df['CTR'] = np.where(sp_df['广告曝光量']>0, sp_df['广告点击数']/sp_df['广告曝光量'], 0)
                sp_df[f'{prefix}销售转化率'] = np.where(sp_df[f'{prefix}会话数']>0, sp_df[f'{prefix}订单商品总数']/sp_df[f'{prefix}会话数'], 0)
                sp_df['预计可售天数'] = np.where(sp_df['预测日销量']>0, sp_df['总供给']/sp_df['预测日销量'], 0)
            ...
            with st.expander("📊 SPU/SKU 矩阵图", expanded=False):
                ...
                m_df = m_df[(m_df[f'{prefix}销售额'] > 0) | (m_df['广告花费'] > 0)]
                if not m_df.empty:
                    m_df['bs'] = pd.to_numeric(m_df[f'{prefix}日均订单'], errors='coerce').clip(lower=1)
                    m_df['订单毛利率'] = pd.to_numeric(m_df['订单毛利率'], errors='coerce')
                    fig = px.scatter(m_df, x=f'{prefix}销售额', y='订单毛利率', size='bs', color='ACOS', hover_name='MSKU' if is_filtered else spu_c, size_max=45, color_continuous_scale='RdYlBu_r')
    ...
    with tab2:
        ...
        with c_c2:
            ...
            if f'{prefix}销售额' in target_df.columns:
                ...
                if not m_a.empty:
                    m_a['订单毛利率'] = pd.to_numeric(m_a['订单毛利率'], errors='coerce')
                    fg2 = px.scatter(m_a, x=f'{prefix}销售额', y='订单毛利率', size=pd.to_numeric(m_a.get(f'{prefix}日均订单',1), errors='coerce').clip(lower=1), color='ACOS', hover_name='MSKU', size_max=40, color_continuous_scale='RdYlBu_r')
    ...
    st.markdown("---")
    st.download_button(label="📥 下载完整【V35·极速引擎全息穿透版.xlsx】", data=st.session_state.processed_excel, file_name=f"V35_极速引擎大盘_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary")
